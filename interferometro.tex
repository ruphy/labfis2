...
Dunque, l'incertezza su $l$ è data dalla relazione
...
errorsarr = [(subd.subs(n=int(dato['n']), k=dato['k'])*10^7).n(digits=4) for dato in dati]
print errorsarr
...
def stampa_dati(wa, header):
  ...
  for i in range(0, len(wa)):
    a = ["%s" % x for x in wa[i]]
    s += "%s & %i & %.2f \\\\" % (join(a, "&"), lambdas[i], errorsarr[i])
  s += r"\end{tabular}"
  return s
\end{sagesilent}
