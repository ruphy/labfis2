...
\begin{center}
 \includegraphics[scale=0.70]{grafici/C3/rct.png}
\end{center}
...
I dati graficati:
\begin{center}
 \includegraphics[scale=0.70]{grafici/C3/rlt.png}
\end{center}
...
plt.xlabel(r"$\nu$ Frequenza $(Hz)$")
plt.grid(True)
plt.savefig("grafici/C3/fasecap.png",dpi=300)
out.pprint()
...
La frequenza di taglio si definisce come $\omega_c = \frac{1}{RC}$. Nel nostro caso $\omega_c = \sage{round(omega,2)}\ Hz$. Il comportamento del circuito varia a seconda di quanto la nostra frequenza $\omega$ si avvicina al valore della frequenza di taglio. \\
...
rlt = np.recfromcsv('dati/C3/RLfun.csv')
deltaphi= rcf['frequenza']*rcf['tempo']*2*n(pi)*10^(-6)
yerr = deltaphi*0.06
xerr= 0
...
var('x,C')
def funz(x, L):
    return arctan(x*2*n(pi)*677/L)
    
def func(P,x):
    return arctan(x*2*n(pi)*677/P[0])
    
mymod = odr.Model(func)
mydata = odr.RealData(freq,deltaphi)

myodr = odr.ODR(mydata, mymod, beta0=[0.0001], maxit=5000)
out = myodr.run()
